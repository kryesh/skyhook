...
const SCRIPT_DESCRIPTION: &str = r#"Run an async JavaScript body with top-level await/return. Native result is {value,console}: value is the returned JSON (null without return), and console is captured console.log text/JSON. The model receives a JobView with this script result. Read saved console at /result/console and returned-value fields under /result/value. No Node, fetch, URL, TextEncoder/TextDecoder, setTimeout/setInterval or recursive script. Built-ins include Date, RegExp, Map/Set, Proxy/Reflect, BigInt, ArrayBuffer, DataView and typed arrays; Uint8Array supports fromBase64/fromHex/toBase64/toHex. Return JSON-compatible values: convert BigInts, dates and typed arrays; undefined, non-finite numbers, functions and cycles fail serialization.
