...
use crate::session::{ModelPurpose, project_history, reconstruct_model_request};
...
impl Fixture {
    ...
    async fn assert_request_replay(&self) {
        let records = self.records().await;
        let mut replay = Vec::new();
        for record in &records {
            if matches!(record.event, SessionEvent::ModelRequested { .. }) {
                let (_, mut request) =
                    reconstruct_model_request(&records, record.sequence).unwrap();
                self.session
                    .runtime
                    .store
                    .hydrate_model_request(&mut request)
                    .await
                    .unwrap();
                replay.push(request);
            }
        }
        assert_eq!(replay, self.requests());
    }
}
...
async fn startup_failure_retries_the_exact_request_without_committing_an_error() {
    ...
    fixture.assert_request_replay().await;
...
async fn failed_partial_text_and_complete_tool_block_are_discarded_before_retry() {
    ...
    let history =
        serde_json::to_string(&project_history(&records, &fixture.session.root).unwrap()).unwrap();
...
async fn child_recovery_keeps_the_same_owner_job_and_does_not_fail_the_agent() {
    ...
    assert!(
        !records.iter().any(|record| record.agent == child
