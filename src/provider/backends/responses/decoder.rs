...
impl Decoder {
    ...
    pub(crate) fn codex(model: String, scope: Scope) -> Self {
        Self {
            allow_omitted_terminal_output: true,
            ..Self::new(model, scope)
        }
    }
...
mod tests {
    ...
    fn function_completion_replaces_partial_identity_with_validated_call() {
        let mut decoder = Decoder::new(
            "model".into(),
            crate::provider::backends::common::tests::scope(),
        );
        decoder
    ...
    fn provisional_function_snapshots_are_discarded_only_on_abnormal_stop() {
        ...
        for (reason, cut) in [
            ...
            let mut decoder = Decoder::codex("model".into(), scope());
