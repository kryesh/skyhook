        ...
        move |context, arguments| {
            ...
            async move {
                    ...
                    .filter(|target| *target != ROOT_TARGET)
                {
                    ...
                    return finish_remote(context, output).await;
                }
...
async fn finish_remote(
    ...
    if !output.stdout.is_empty() {
        context
            .progress("stdout", serde_json::json!({"text": output.stdout}))
            .await?;
    }
    if !output.stderr.is_empty() {
        context
            .progress("stderr", serde_json::json!({"text": output.stderr}))
            .await?;
    }
...
mod tests {
    ...
    async fn nonzero_is_success_and_timeout_persists_partial_output() {
        ...
        register(&mut builder, RemoteManager::new(TargetRegistry::default())).unwrap();
