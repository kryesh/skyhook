//! Building and validating the embedded remote shims.
...
pub(super) fn dispatch() -> bool {
        ...
        || basename.ends_with("dlltool")
    {
        ...
        let manifest_dir = PathBuf::from(args.next().expect("missing worker manifest directory"));
...
fn worker(manifest_dir: &Path, out_dir: &Path, outer_profile: &str) {
    ...
    for target in SHIM_TARGETS {
        ...
        write_if_changed(&destination, &bytes)
            .unwrap_or_else(|error| panic!("could not publish {}: {error}", destination.display()));
    }
