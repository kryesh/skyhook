...
Configure Model Context Protocol servers in the top-level **`mcp`** mapping, with one named
mapping per server. It defaults to empty: no MCP connections, tools, or MCP prompt
...
belong in YAML. Missing or non-Unicode variables, invalid header names, and values that cannot be
...
from a workspace `.skyhook/config.yaml` overlay. Review workspace config before use. They run during
...
and before any MCP tool can be exposed or invoked. Each server's `capabilities` sequence defaults to
